...
const WriteArticle = () => {
    ...
    const articleLength = [
        ...
        { length: 16000, text: 'Long(1200-1600 words)' },
    ]
                ...
                <div className='mt-3 flex flex-wrap gap-3 sm:max-w-9/11'>
                    {
                        articleLength.map((item, index) => (
                            <span onClick={() => setSelectedLength(item)} key={index} className={`text-sm py-1 px-4 border rounded-full cursor-pointer ${selectedLength.text === item.text ? "bg-blue-50 text-blue-700" : "text-gray-500 border-gray-300"}`}>{
                                item.text
                            }</span>
                        ))
                    }
